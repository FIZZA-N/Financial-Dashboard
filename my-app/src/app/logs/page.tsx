...
export default function LogsPage() {
  ...
  const [delAction, setDelAction] = useState<'All'|'CREATE'|'UPDATE'|'DELETE'|'VIEW'>('All');
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/users/audit-logs', { params: { limit } });
      ...
      if (entityFilter.trim()) items = items.filter(l => l.entityType.toLowerCase().includes(entityFilter.trim().toLowerCase()));
      if (userQuery.trim()) {
        items = items.filter(l => {
          if (typeof l.userId === 'object') {
            const obj: any = l.userId;
            return (obj.name?.toLowerCase().includes(userQuery.toLowerCase()) || obj.email?.toLowerCase().includes(userQuery.toLowerCase()));
          }
          return (l.userName || '').toLowerCase().includes(userQuery.toLowerCase());
        });
      }
      setLogs(items);
  ...
  const applyDelete = async () => {
    ...
    try {
      ...
      setDelStart(''); setDelEnd(''); setDelAction('All'); setDelEntity('');
      await fetchLogs();
      const count = data?.deletedCount ?? 0;
      toast.success(`Deleted ${count} log(s)`);
    } catch (e) {
  ...
  const enhancedLogs = useMemo(() => logs.map((l) => ({
    ...l,
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 rounded-xl shadow-lg border border-gray-200 p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4 md:items-end">
            ...
            <div>
              ...
              <input value={entityFilter} onChange={(e)=>setEntityFilter(e.target.value)} placeholder="e.g., Order, User" className="w-full px-3 py-2 border rounded-md" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">User</label>
              <input value={userQuery} onChange={(e)=>setUserQuery(e.target.value)} placeholder="name or email" className="w-full px-3 py-2 border rounded-md" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={String(limit)} onChange={(e)=>setLimit(parseInt(e.target.value))} className="w-full px-3 py-2 border rounded-md">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={()=>{setActionFilter('All');setEntityFilter('');setUserQuery('');setStartDate('');setEndDate('');fetchLogs();}} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md">Reset</button>
            <div className="flex-1" />
            <button onClick={()=>setShowDeleteModal(true)} className="px-4 py-2 bg-red-600 text-white rounded-md">Delete Logs by Filter</button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            enhancedLogs.map((log) => {
              ...
              const header = `${log.action} ${log.entityType}${log.entityId ? ` (${log.entityId})` : ''}`;
              const diffs = log.diffs;
              
              return (
                <div key={log._id} className="bg-white rounded-xl shadow border border-gray-200">
                  <div className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`h-9 w-9 rounded-lg flex items-center justify-center text-white font-semibold ${
                        log.action==='CREATE' ? 'bg-emerald-600' : log.action==='UPDATE' ? 'bg-blue-600' : log.action==='DELETE' ? 'bg-rose-600' : 'bg-gray-600'
                      }`}>
                        {log.action[0]}
                      </div>
                      <div>
                        <div className="font-semibold text-gray-900">{header}</div>
                        <div className="text-sm text-gray-500">{userName} • {when}</div>
                      </div>
                    </div>
                  </div>
                  {diffs.length > 0 && (
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-gray-50">
                              ...
                              <th className="px-4 py-2 text-left text-gray-600">Before</th>
                              <th className="px-4 py-2 text-left text-gray-600">After</th>
                            </tr>
                          </thead>
                  ...
                  {diffs.length === 0 && (
                    <div className="px-4 pb-4 text-sm text-gray-500">No field changes recorded.</div>
                  )}
                </div>
              );
            })
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-xl p-6">
              <h3 className="text-lg font-bold mb-4">Delete Logs by Filter</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Action</label>
                  <select value={delAction} onChange={(e)=>setDelAction(e.target.value as any)} className="w-full px-3 py-2 border rounded-md">
                    <option>All</option>
                    <option value="CREATE">CREATE</option>
                    <option value="UPDATE">UPDATE</option>
                    <option value="DELETE">DELETE</option>
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Entity</label>
                  <input value={delEntity} onChange={(e)=>setDelEntity(e.target.value)} placeholder="e.g., Order" className="w-full px-3 py-2 border rounded-md" />
                </div>
              </div>
              ...
              <div className="flex justify-between">
                <button onClick={()=>{setDelStart('');setDelEnd('');setDelAction('All');setDelEntity('');}} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-md hover:bg-gray-200">Clear</button>
                <div className="flex gap-2">
                  ...
                  <button disabled={deleting} onClick={applyDelete} className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 disabled:opacity-50">
